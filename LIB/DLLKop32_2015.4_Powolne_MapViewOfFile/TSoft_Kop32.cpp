...
//---------------------------------------------------------------------------
//F:\ISO\NAS4Free-x86-LiveUSB-9.3.0.2.1349.img
#include <windows.h>
#include <winbase.h>
...
register TSoft_Kop32 *CallerKop32 = (TSoft_Kop32*)a;

 while ((CallerKop32->WriteFileThreadControl & 2) == 0) {
	if ((CallerKop32->WriteFileThreadControl & 1) != 0) {
		int i =  CallerKop32->mem_write_index;
		CallerKop32->file.dst.buffer.Size = CallerKop32->mem_buffer[i].Size;
		CallerKop32->file.dst.buffer.Offset = CallerKop32->file.dst.Read % 65536;
		CallerKop32->file.dst.buffer.Lptr = MapViewOfFile(CallerKop32->file.dst.Hand_map, FILE_MAP_WRITE, 0,
														  CallerKop32->file.dst.Read - CallerKop32->file.dst.buffer.Offset, CallerKop32->file.dst.buffer.Size + CallerKop32->file.dst.buffer.Offset);
		if (CallerKop32->file.dst.buffer.Lptr==NULL)
		   {CallerKop32->WriteFileThreadControl = 4;
		   }
		CallerKop32->file.dst.buffer.Lptr =(void*)((ULONG)CallerKop32->file.dst.buffer.Lptr + CallerKop32->file.dst.buffer.Offset);
		ptrEql(CallerKop32->file.dst.buffer.Lptr, CallerKop32->mem_buffer[i].Lptr,CallerKop32->mem_buffer[i].Size);
		UnmapViewOfFile((void*)((ULONG)CallerKop32->mem_buffer[i].Lptr - CallerKop32->mem_buffer[i].Offset));
		if (CallerKop32->options.Cached==0)
		FlushViewOfFile(CallerKop32->file.dst.buffer.Lptr,CallerKop32->file.dst.buffer.Size);
		UnmapViewOfFile((void*)((ULONG)CallerKop32->file.dst.buffer.Lptr - CallerKop32->file.dst.buffer.Offset));
		CallerKop32->file.dst.Read += CallerKop32->file.dst.buffer.Size;
		CallerKop32->progress.dst.all.Read += CallerKop32->file.dst.buffer.Size;
		CallerKop32->progress.dst.one.Read  = CallerKop32->file.dst.Read;
		CallerKop32->progress.dst.one.Size  = CallerKop32->file.dst.Size;
		CallerKop32->FOnBreak(onPROGRESS,4);
		CallerKop32->WriteFileThreadControl = 0;
		SwitchToThread();
		}
else
		{
...
void  __stdcall TSoft_Kop32::set_Password(char *ausrpassword)
{
strEql(password.lpUsr,ausrpassword);
strEql(password.lpEnc,ausrpassword); password.Size = strLen(ausrpassword);

int  pos;  __int64 kod = 0;
for (pos = password.Size-1; pos!=-1; pos--)
	 {kod +=password.lpEnc[pos];
	 }
for (pos = password.Size-1; pos!=-1; pos--)
    {password.lpEnc[pos]^= (kod % (pos+1)) ^ pos;
	 }
}
//---------------------------------------------------------------------------

char *__stdcall TSoft_Kop32::get_Password()
{
return password.lpUsr;
}
//---------------------------------------------------------------------------
...
int __stdcall TSoft_Kop32::Execute(char *AlpDst, char *AlpSrc) {
...
strEql(templp,templpDst);
strupr(templp);
if (Pharser(templp,"BUFORUJ",NULL,xpos))
   {options.Cached = true;
    strDelEnd(templpDst,xpos[0],xpos[3]);
   }
options.Block = 1024 * 1024;
strEql(templp,templpDst);
strupr(templp);
...
file.src.Hand_map = INVALID_HANDLE_VALUE;
file.src.Read = 0;
file.src.Size = list.src.Main.Items[aindex]->data_size[SIZE];
file.dst.Hand = INVALID_HANDLE_VALUE;
file.dst.Hand_map = INVALID_HANDLE_VALUE;
file.dst.Read = 0;
file.dst.Size = list.dst.Main.Items[aindex]->data_size[SIZE];
	//
...
FOnBreak(anaction|isFILE|onPROGRESS,0);
	...
	file.dst.Hand = CreateFile(list.dst.Main.Items[aindex]->text,
									 ...
									 FILE_ATTRIBUTE_ARCHIVE|FILE_FLAG_SEQUENTIAL_SCAN, file.src.Hand);
	...
	if (anaction==opKODUJ)
		...
		file.dst.Hand_map = CreateFileMapping(file.dst.Hand, NULL,
													 PAGE_READWRITE|SEC_RESERVE|SEC_NOCACHE,
													 0,sizeof(__header),NULL);
		    file.dst.buffer.Lptr = MapViewOfFile(file.dst.Hand_map,FILE_MAP_WRITE,0,0,sizeof(__header));
...
progress.dst.all.Read += sizeof(__header);
	...
	if (anaction==opDEKODUJ)
		...
		file.src.Read = 0;
		file.src.Size = GetFileSize(file.src.Hand, NULL);
		file.src.Hand_map = CreateFileMapping(file.src.Hand, NULL,
													 PAGE_READONLY|SEC_RESERVE|SEC_NOCACHE,
													 0,sizeof(__header),NULL);
		file.src.buffer.Lptr = MapViewOfFile(file.src.Hand_map, FILE_MAP_READ,0,0,sizeof(__header));
...
progress.dst.all.Read += 0;
	...
	if (file.src.Hand_map==INVALID_HANDLE_VALUE)
		{goto ProcedeERRORIO;
		}
	 // Map Dst >>
	file.dst.Hand_map = CreateFileMapping(file.dst.Hand, NULL,
	...
	if (file.dst.Hand_map==INVALID_HANDLE_VALUE)
		{goto ProcedeERRORIO;
		}
	//
	// START MOVE
	...
	//
	if (anaction==opKOPIUJ) {
  for (mem_count = 8, mem_read_index = 0, mem_write_index = 0; options.Stop==0 && file.src.Read < file.src.Size;)
		{
		//
		//
	...
	if (file.src.buffer.Size > options.Block) file.src.buffer.Size = options.Block;
		...
		file.src.buffer.Lptr = MapViewOfFile(file.src.Hand_map, FILE_MAP_READ, 0,
...
progress.src.one.Size  = file.src.Size;
		...
		while (WriteFileThreadControl & 1) { Sleep(1); FOnBreak(onPROGRESS,0); } // dopuki system nie zakoñczy poprzedniej procedury zapisu czekaj
		   if (WriteFileThreadControl & 4) goto ProcedeERRORIO; // zapisywanie zakoñczone ale niepoprawnie
...
//      SwitchToThread();
		...
		file.dst.buffer.Lptr = MapViewOfFile(file.dst.Hand_map, FILE_MAP_WRITE, 0,
...
progress.dst.all.Read += file.dst.buffer.Size;
  ...
  if (file.src.buffer.Size > options.Block) file.src.buffer.Size = options.Block;
		...
		if (options.Methods & mtSXQ)
			{Cript_SXQ(mem_buffer[mem_read_index].Lptr,mem_buffer[mem_read_index].Size,password.Size);
			}
		// Xorowanie
		if (options.Methods & mtXOR)
			{Cript_XOR(mem_buffer[mem_read_index].Lptr,mem_buffer[mem_read_index].Size,password.lpEnc,password.Size);
			}
...
progress.dst.all.Read += sizeof(__block_header);
  ...
  for (;options.Stop==0 && file.src.Read < file.src.Size;)
		...
		if (b_hdr.methods & mtXOR)
			{UnCript_XOR(mem_buffer[mem_read_index].Lptr,mem_buffer[mem_read_index].Size,password.lpEnc,password.Size);
			}
		// DEQDRowanie
		if (b_hdr.methods & mtSXQ)
			{UnCript_SXQ(mem_buffer[mem_read_index].Lptr,mem_buffer[mem_read_index].Size,password.Size);
			}
		// DEkompresja ARI
		if (b_hdr.methods & mtARI)
			{mem_buffer[mem_read_index-1].Lptr = ptrAlloc(mem_buffer[mem_read_index-1].Size+1);
...
//------------------------------------------
    ...
    WriteFileThreadControl = 0;
		...
		CloseHandle(file.dst.Hand_map);
	SetFilePointer(file.dst.Hand, file.dst.Read, 0, FILE_BEGIN); SetEndOfFile(file.dst.Hand);
