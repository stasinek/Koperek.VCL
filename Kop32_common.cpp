...
if (strstr(temp1,"/COPY")!=NULL)
    Application->Icon->Handle = LoadIcon(hApp,"MAINICON1");
else
...
if (useWizzard==true) { //---------------------
    // If you use wizzard, then just ignore ARGTS arguments, or get the other way via WinAPI
    ...
    if (clip_ptr!=NULL)
        { // if its initialized you get data, if not initialize an empty string
        commandSrc = (char*)malloc(strlen(clip_ptr)+4);
        strcpy(commandSrc,clip_ptr);
        }
    else
        {
        commandSrc = (char*)calloc(4,1);
        }
    // commandDst wa already copied, temp1 was already copied, now copy content to helper temp2 [for DEBUG]:
